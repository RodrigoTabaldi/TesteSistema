...
export default function Login() {
  ...
  const temContas = state.users.length > 0;
  ...
  return (
    <div>
      ...
      {temContas && (
        <button
          onClick={() => setValue("email", state.users[state.users.length - 1]?.email ?? "")}
          className="mt-3 w-full text-center text-[12.5px] text-fg-dim hover:text-fg-muted"
        >
          Usar a última conta criada ({state.users[state.users.length - 1]?.email})
        </button>
      )}
