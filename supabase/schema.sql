-- ============================================================
-- KRONOS — Postgres schema (referência, NÃO usado pelo app em produção)
--
-- O app hoje persiste no Firestore (ver lib/cloud.ts e /firestore.rules)
-- como um snapshot JSON por usuário. Este schema fica como o desenho de
-- uma futura migração para dados normalizados/relacionais — útil quando
-- o produto precisar de consultas/relatórios que um documento único não
-- resolve bem. Rodar isto no SQL Editor do Supabase não conecta a nada:
-- é só a definição de tabelas + RLS para quando essa migração acontecer.
-- ============================================================
